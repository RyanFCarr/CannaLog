...
const PlantList: React.FC = () => {
	...
	useEffectOnce(() => {
		const getPlants = async () => {
			try {
				const res = await get<PlantDto[]>(`${import.meta.env.VITE_API_BASE_URL}/plant`);
				if (res.parsedBody) {
	...
	return <Table data={plants} columnHeaders={['Name', 'Strain', 'Status']} onRowClick={(plant: Plant) => navigate(`/plants/${plant.id}`)} />;
};
