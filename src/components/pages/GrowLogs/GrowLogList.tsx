	...
	return (
		<>
			<Routes>
				<Route
					index
					element={<BasePage title="Grow Log List" Body={<GrowLogList plantId={plantId} />} Footer={<Footer plantId={plantId} />} />}
				/>
...
const GrowLogList: React.FC<GrowLogListProps> = ({ plantId }: GrowLogListProps) => {
	...
	useEffectOnce(() => {
		const getGrowLogs = async () => {
			try {
				const res = await get<GrowLogDto[]>(`${import.meta.env.VITE_API_BASE_URL}/GrowLog`);
				if (res.parsedBody) {
...
const Footer: React.FC<GrowLogListFooterProps> = ({ plantId }: GrowLogListFooterProps) => {
	return (
		<Box display="flex" justifyContent="space-around" mt={1.5}>
			<Button href={`/plants/${plantId}/growlogs/add`} variant="contained" startIcon={<AddIcon />}>
				Add Grow Log
			</Button>
		</Box>
	);
