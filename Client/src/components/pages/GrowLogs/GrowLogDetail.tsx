...
const Layout: React.FC = () => {
	...
	if (!plantId)
		return (
			<div>
				Plant Id is missing. You should report a bug! <a href="https://github.com/RyanFCarr/CannaLog/issues">report a bug</a>
			</div>
		);
	if (!Number.parseInt(plantId))
		return (
			<div>
				Plant Id is not an integer. You should report a bug: <a href="https://github.com/RyanFCarr/CannaLog/issues">report a bug</a>
			</div>
		);

	const [viewMode, setViewMode] = useState<ViewMode>(growLogId ? ViewMode.VIEW : ViewMode.ADD);
	...
	return (
		<>
			...
			>
				...
				/>
				<MultiSelectChip label="Tags" chips={['Res Change', 'Flush']} setSelected={setTags} />
			</Box>
		</>
	);
};
	...
	return (
		<Box display="flex" justifyContent="space-around" sx={{ mt: 1.5, mb: 2 }}>
			{viewMode === ViewMode.ADD && (
				<Button variant="contained" color="error" href={`/plants/${plantId}/growlogs`}>
					Discard
				</Button>
			)}
